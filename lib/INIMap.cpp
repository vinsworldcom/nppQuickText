	...
	while (getline(configFile, lineBuffer)) {
		...
		// e.g. if=if ()
			...
			if (equalSignIndex != string::npos) {
				...
				for (vector<string>::const_iterator i = curSections.begin(); i != curSections.end(); i++)
...
bool INIMap::WriteFile (const TCHAR* filename) const {
	...
	for (inimap::const_iterator i = data.begin(); i != data.end(); i++) {
		...
		f << '[' << i->first << ']' << endl;
		for (keymap::const_iterator j = i->second.begin(); j != i->second.end(); j++) {
...
bool INIMap::Exists(cstring &section) const {
	if(data.find(section) != data.end()) return true;
	return false;
}
...
bool INIMap::Exists(cstring &section, cstring &tag) const {
	...
	if(i == data.end()) return false;
	if(i->second.find(tag) != i->second.end()) return true;
	return false;
}
...
TagList INIMap::queryTags(cstring & section, cstring & tag)
{
	...
	cmatch matchResults;
	//+@TonyM: "([\\S]*" + tag + "([\\S]*)" -> "^" + tag + "([\\S]*)"
	regex tagsRegex("^" + tag + "([\\S]*)"); // match any string with tag as substring
