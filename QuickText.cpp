...
HFONT verdanaFont = CreateFont (15, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, _T("Verdana"));
		...
		case DLL_PROCESS_ATTACH:
			{
				...
				/*funcItems[1]._pFunc = refreshINIMap;
				strcpy(funcItems[1]._itemName, "Refresh Configuration");
				funcItems[1]._init2Check = false;
				funcItems[1]._pShKey = new ShortcutKey;
				funcItems[1]._pShKey->_isAlt = false;
				funcItems[1]._pShKey->_isCtrl = true;
				funcItems[1]._pShKey->_isShift = true;
				funcItems[1]._pShKey->_key = 0x51; // Q*/
...
extern "C" __declspec(dllexport) void beNotified(SCNotification *notifyCode) {
	switch (notifyCode->nmhdr.code) {
		...
		case SCN_MODIFIED:
			{
				if (cQuickText.editing) {
					if (notifyCode->modificationType & SC_MOD_INSERTTEXT) {
						for (vector<int>::iterator i = cQuickText.hotSpotsPos.begin(); i != cQuickText.hotSpotsPos.end(); i++)
		...
		case SCN_UPDATEUI:
			{
				int currpos;
				currpos = static_cast<int>(SendMessage(getCurrentHScintilla(), SCI_GETCURRENTPOS, 0, 0));
				bool r = true;
				for (unsigned int i = 0; i < cQuickText.hotSpotsPos.size(); i++)
					if (currpos >= cQuickText.hotSpotsPos[i] && currpos <= cQuickText.hotSpotsPos[i] + cQuickText.hotSpotsLen[i])
						r = false;
				if (r)
					clear();
			}
...
bool isValidKey(const char *key) {
	string skey = key;
	if (skey.find_first_not_of(allowedChars) != string::npos)
		return false;
	return true;
}
...
void _refreshINIFiles()
{
	...
	std::string ini_file_path		= QTString::ws2s(config_dir) + "\\QuickText.conf.ini";
	std::string ini_file_section	= "general";
	
	//+@TonyM: Reads allowedChars value from config file on each config refresh
	std::string ini_allowedChars = CIniFile::GetValue("allowedChars", ini_file_section, ini_file_path);
	...
	std::string ini_lang_menu = CIniFile::GetValue("lang_menu", ini_file_section, ini_file_path);
...
{
	_refreshINIFiles();
	MessageBox(nppData._nppHandle, _T("QuickText.ini and QuickText.conf.ini files reloaded!"), _T("QuickText"), MB_OK | MB_ICONINFORMATION);
}

void openConfigFile()
{
	basic_string<TCHAR> file_path = config_dir + _T("\\QuickText.conf.ini");
	SendMessage(nppData._nppHandle, NPPM_DOOPEN, 0, (LPARAM)file_path.c_str());
}
...
void QuickText() {
	...
	textSelectionStart = static_cast<int>(SendMessage(scintilla, SCI_GETSELECTIONSTART, 0, 0));
	textSelectionEnd = static_cast<int>(SendMessage(scintilla, SCI_GETSELECTIONEND, 0, 0));
	...
	*/
	// get 'text' location
	curPos = static_cast<int>(SendMessage(scintilla, SCI_GETCURRENTPOS, 0, 0));
	startPos = static_cast<int>(SendMessage(scintilla, SCI_WORDSTARTPOSITION, curPos, (LPARAM)true));
	endPos = static_cast<int>(SendMessage(scintilla, SCI_WORDENDPOSITION, curPos, (LPARAM)true));
...
// 	}
	...
	if (tagInCurrentLang || tagInGlobalLang)
	{
		...
		if (Config.indenting)
		{
			...
			char temp[128];
			int lineNumber = static_cast<int>(SendMessage(scintilla, SCI_LINEFROMPOSITION, curPos, 0));
			int startline = static_cast<int>(SendMessage(scintilla, SCI_POSITIONFROMLINE, lineNumber, 0));
	...
	else if ( tagList.size() > 0 )
	{
		...
		for (TagList::const_iterator index = tagList.begin(); index != tagListEnd; index++)
		{
			tagList_ss << *index;
		...
		SendMessage(scintilla, SCI_AUTOCSHOW, (WPARAM) strlen(tag), (LPARAM)newList.c_str());
...
void jump(HWND &scintilla) {
	if(cQuickText.hotSpotsPos.size() != 0)
	{
		...
		SendMessage(scintilla, SCI_SETCURRENTPOS, (WPARAM)cQuickText.hotSpotsPos[cQuickText.cHotSpot] + cQuickText.hotSpotsLen[cQuickText.cHotSpot], 0);
	}
	else
	{
		cQuickText.editing = false;
	...
	case WM_INITDIALOG:
		{
			...
			{
				SendMessageA(ConfigWin.langCB, CB_INSERTSTRING, -1, (LPARAM) lang_menu.at(i).c_str());
			}
	...
	case WM_COMMAND:
		...
		case IDOK:{
			...
			if (ConfigWin.changed == true &&  
				MessageBox(hwndDlg, _T("Do you want to save changes?"), _T("Warning"), MB_YESNO | MB_ICONWARNING))
			{
				// add tag/substitution to DB before exiting
				exiting = DlgConfigProc(hwndDlg, (UINT) WM_COMMAND, (WPARAM)((BN_CLICKED << 16) | ( IDADD)), 0);
			}
		...
		case IDCANCEL:
			...
			{
				if (MessageBox(hwndDlg, _T("Are you sure you want to discard changes?"), _T("Warning"), MB_YESNO | MB_ICONWARNING) == IDYES)
					EndDialog(hwndDlg, wParam);
			}
			else
			{
		...
		case IDLANG:
			...
			case LBN_SELCHANGE:
				...
				// Fill the tags list
				for (keymap::const_iterator i = tags_replica[langss.str()].begin(); i != tags_replica[langss.str()].end(); i++)
				{
		...
		case IDADD:
			{
				switch(HIWORD(wParam))
				{
					case BN_CLICKED:
						...
						if(textLength == 1){
							MessageBox(hwndDlg, _T("Invalid Substitution Text! Please check all input fields."), _T("Warning"), MB_OK | MB_ICONWARNING);
							return FALSE;
						}

						// get the old and new tagnames.
						tag_Listbox_Index = (int) SendMessage(ConfigWin.tag, LB_GETCURSEL, 0, 0);
						SendMessageA(ConfigWin.tag, LB_GETTEXT, (WPARAM) tag_Listbox_Index, (LPARAM) oldtag);
						SendMessageA(ConfigWin.tagname, WM_GETTEXT, (WPARAM) SZ_TAG, (LPARAM) newtag);

						// validating tag
						if (strlen(newtag) == 0)
						{
							MessageBox(hwndDlg, _T("Invalid Tag Name! Please check all input fields."), _T("Warning"), MB_OK | MB_ICONWARNING);
							return FALSE;
						}

						// dynamically allocating space for substitution text
						substitutionTxt_ptr = (char*)malloc((sizeof(char)*((size_t)textLength)));
						...
						SendMessageA(ConfigWin.text, WM_GETTEXT, (WPARAM) textLength, (LPARAM) substitutionTxt_ptr);
						...
						// Refresh
						SendMessage(ConfigWin.tag, LB_RESETCONTENT, 0, 0);
						for (keymap::const_iterator i = tags_replica[lang_ss.str()].begin(); i != tags_replica[lang_ss.str()].end(); i++)
							SendMessageA(ConfigWin.tag, LB_ADDSTRING, 0, (LPARAM) i->first.c_str());
						newtag_index = static_cast<int>(SendMessageA(ConfigWin.tag, LB_FINDSTRING, (WPARAM) -1, (LPARAM) newtag));
		...
		case IDDEL:
			switch(HIWORD(wParam))
			{
				case BN_CLICKED:
					...
					{
						MessageBox(hwndDlg, _T("No tag name is selected."), _T("Warning"), MB_OK | MB_ICONWARNING);
						return TRUE;
					}

					SendMessageA(ConfigWin.tag, LB_GETTEXT, (WPARAM) selectedTag, (LPARAM) tagname);
